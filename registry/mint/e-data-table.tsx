...
export type DataTableRowStatus =
  | "done"
  | "in-process"
  | "cancelled"
  | "pending";
      ...
      <DropdownMenuContent align="start" className="w-44">
        ...
        {options.map((opt) => {
          ...
          return (
            ...
            >
              {Icon && (
                <Icon className={`mr-1 size-3.5 ${opt.iconClassName ?? ""}`} />
              )}
              {opt.label}
            </DropdownMenuCheckboxItem>
          );
...
const StatusCell = ({ status }: { status: DataTableRowStatus }) => {
  if (status === "done") {
    return (
          ...
          <div className="flex items-center gap-2">
            <Avatar className="size-7 shrink-0">
              {row.original.avatarSrc && (
                <AvatarImage src={row.original.avatarSrc} alt={name} />
              )}
